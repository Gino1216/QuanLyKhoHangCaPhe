...
public class ProductForm extends javax.swing.JInternalFrame {
    ...
    public void loadDataToTable() {
        try {
            PCDao mtdao = new PCDao();
            ArrayList<PC> armt = mtdao.selectAll();
...
//        a.setVisible(true);
    ...
    public void xoaMayTinhSelect() {
        ...
        if (luaChon == JOptionPane.YES_OPTION) {
            PC remove = getMayTinhSelect();
            PCDao.getInstance().deleteTrangThai(remove.getMaMay());
        }
        loadDataToTable();
    }

    public PC getMayTinhSelect() {
        int i_row = tblSanPham.getSelectedRow();
        PC acc = PCDao.getInstance().selectById(tblModel.getValueAt(i_row, 0).toString());
        return acc;
    }

    public void loadDataToTableSearch(ArrayList<PC> result) {
        try {
            tblModel.setRowCount(0);
            for (PC i : result) {
                String loaimay;
